...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        batch_size = model_config.parameters.get('batch_size', {
            'small': 16,
            'medium': 8,
            'large': 4
        }.get(model_type, 8))
        
        num_epochs = model_config.parameters.get('num_epochs', {
            'small': 5,
            'medium': 3,
            'large': 8
        }.get(model_type, 3))
        
        num_iterations = model_config.parameters.get('num_iterations', {
            'small': 8,
            'medium': 20,
            'large': 12
        }.get(model_type, 20))
        ...
        accuracy = sum(p == l for p, l in zip(predictions, dataset["test"]["label"])) / len(predictions)
