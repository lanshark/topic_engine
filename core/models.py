...
class BaseModel(models.Model):
    ...
    def is_healthy(self) -> bool:
        ...
        if self.error_count >= 3:  # error_count is unknown...
            return False
        # NOTE: last_success is unknown...
        if self.last_success and (timezone.now() - self.last_success) > timedelta(
            days=2,
        ):
            return False
        return True
...
class ModelConfig(BaseModel):
    ...
    def update_training_metrics(self, num_examples: int, accuracy: float | None = None):
