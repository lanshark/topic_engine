...
from .models import Source, Topic, Content, TopicPrediction, TrainingExample, ModelConfig
...
class SourceAdmin(admin.ModelAdmin):
    list_display = ('name', 'url', 'source_type', 'active', 'health_status', 
    ...
    def health_status(self, obj):
        ...
        return format_html(
            '<span style="color: red;">●</span> Unhealthy'
        )
    ...
    def import_opml(self, request):
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                self.message_user(
                    request, 
                    f'Successfully imported {len(sources)} RSS feeds from OPML'
                )
        ...
        return render(
            request,
            'admin/core/source/opml_upload.html',
            {'form': form}
        )
...
class ContentAdmin(GISModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('source', 'url', 'title', 'raw_content')
        }),
        ('Processing', {
            'fields': ('processed', 'processed_content', 'processing_version', 
                      'processing_errors')
        }),
        ('Geographic', {
            'fields': ('location', 'geo_context')
        }),
        ('Metadata', {
            'fields': ('publish_date', 'authors', 'metadata')
        })
    )
