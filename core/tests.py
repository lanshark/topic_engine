...
class SourceModelTests(TestCase):
    def setUp(self):
        self.rss_source = Source.objects.create(
            url="http://example.com/feed.xml", name="Test RSS Feed", source_type="rss"
        )
        self.page_source = Source.objects.create(
            url="http://example.com/news",
            name="Test Page Source",
            source_type="page",
            selectors={"title": "h1.title", "content": "div.article-content"},
        )
    ...
    def test_next_check_calculation(self):
        ...
        self.assertAlmostEqual(
            self.rss_source.next_check_due().timestamp(), expected_next.timestamp(), delta=1
        )
...
class TopicModelTests(TestCase):
    def setUp(self):
        self.parent = Topic.objects.create(
            name="Parent Topic", description="Parent topic for testing"
        )
        self.child = Topic.objects.create(
            name="Child Topic", description="Child topic for testing", parent=self.parent
        )
        self.grandchild = Topic.objects.create(
            name="Grandchild Topic", description="Grandchild topic for testing", parent=self.child
        )
    ...
    def test_hierarchy_path(self):
        ...
        self.assertEqual(self.grandchild.path, "parent-topic/child-topic/grandchild-topic")
...
class ContentModelTests(TestCase):
    def setUp(self):
        self.source = Source.objects.create(
            url="http://example.com/feed.xml", name="Test Source", source_type="rss"
        )
    ...
    def test_geographic_features(self):
        ...
        self.content.geo_context = {"city": "San Francisco", "state": "CA", "country": "USA"}
        self.content.save()

        # Test spatial query
        nearby = Content.objects.filter(location__distance_lte=(location, 5000))  # 5km radius
    ...
    def test_topic_scoring(self):
        ...
        self.assertEqual(self.content.topicscore_set.first().score, 0.85)
...
<opml version="1.0">
  ...
  <body>
    ...
    def test_opml_import(self):
        ...
        self.assertTrue(Source.objects.filter(url="http://example.com/feed1.xml").exists())
        self.assertTrue(Source.objects.filter(url="http://example.com/feed2.xml").exists())
