...
class ArchiveSource:
    ...
    async def get_archived_content(
        self, url: str, client: httpx.AsyncClient, config: FetcherConfig
    ) -> Optional[Dict]:
    ...
    async def get_archived_content(
        ...
        try:
            ...
            response = await client.get(
                api_url, headers=headers, timeout=30.0, follow_redirects=True
            )
...
class ArchiveStrategy(ContentFetchStrategy):
    ...
    async def fetch(self, url: str, config: FetcherConfig) -> FetchResult:
        ...
        for archive in self.archives:
            try:
                if archived := await archive.get_archived_content(url, self.client, config):
        ...
        return self.create_result(
            content=None, quality=ContentQuality.EMPTY, error="No archive content available"
        )
