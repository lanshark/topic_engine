...
def article_list(request, source_id):
    ...
    articles = Content.objects.filter(source=source).select_related('source').order_by('-publish_date')
    ...
    for article in articles:
        ...
        try:
            ...
            articles_with_predictions.append({
                ...
                'title': article.title,
                'url': article.url,
                'date': article.publish_date.strftime('%Y-%m-%d %H:%M') if article.publish_date else 'Unknown',
    ...
    return render(request, 'sources/article_list.html', {
        'source': source,
